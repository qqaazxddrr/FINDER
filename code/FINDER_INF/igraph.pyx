...
cdef class py_Graph:
    ...
    def __cinit__(self,*arg):
        ...
        if len(arg)==0:
            ...
            edge_w = np.array([float(x) for x in arg[4]], dtype=np.float32)
            node_w = np.array([float(x) for x in arg[5]], dtype=np.float32)
            self.reshape_Graph(_num_nodes,  _num_edges,  edges_from,  edges_to, edge_w, node_w)
        else:
            print('Error：py_Graph类为被成功初始化，因为提供参数数目不匹配，参数个数为0或6。')
    ...
    @property
    def edge_w_list(self):
        return deref(self.inner_graph).edge_w_list

    @property
    def node_w_list(self):
        return deref(self.inner_graph).node_w_list

    cdef reshape_Graph(self, int _num_nodes, int _num_edges, int[:] edges_from, int[:] edges_to, float[:] edge_w, float[:] node_w):
        cdef int *cint_edges_from = <int*>malloc(_num_edges*sizeof(int))
        cdef int *cint_edges_to = <int*>malloc(_num_edges*sizeof(int))
        cdef float *cfloat_edge_w = <float*>malloc(_num_edges*sizeof(float))
        cdef float *cfloat_node_w = <float*>malloc(_num_nodes*sizeof(float))
        cdef int i
        ...
        for i in range(_num_edges):
            cfloat_edge_w[i] = edge_w[i]
        for i in range(_num_nodes):
            cfloat_node_w[i] = node_w[i]

        self.inner_graph = shared_ptr[Graph](new Graph(_num_nodes,_num_edges,&cint_edges_from[0],&cint_edges_to[0], &cfloat_edge_w[0], &cfloat_node_w[0]))
        free(cint_edges_from)
        free(cint_edges_to)

    def reshape(self,int _num_nodes, int _num_edges, int[:] edges_from, int[:] edges_to, float[:] edge_w, float[:] node_w):
        self.reshape_Graph(_num_nodes, _num_edges, edges_from, edges_to, edge_w, node_w)
